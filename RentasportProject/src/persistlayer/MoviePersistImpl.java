...
public class MoviePersistImpl {
	...
	public ResultSet getRSBundleTypes(String imgPth)
	{
		DbAccessImpl da= new DbAccessImpl();
		Connection con= da.connect();
		String query="Select * from bundles where imagePath='"+ imgPth +"'";
	...
	public ResultSet getRSInsertSportPkg(String custId, String bundleName, String qty, String time)
	{
		DbAccessImpl da= new DbAccessImpl();
		Connection con= da.connect();
		String query="INSERT INTO cart VALUES((select customerId from user where name = '"+ custId +"'),(select orderNum from orders JOIN user ON user.customerId=orders.customerId where user.name= '"+ custId +"' and orders.orderStatus= 'open'), '"+bundleName+"',"+ qty + "," +time+")";
		da.create(con, query);	
